function [stable] = check_stability(c, dt, dh, alpha_abs, order, xi, nu, fourier, nu_fourier)
    ...
    elseif order == 1 && fourier == true
        ...
        % expected max eig of B
        max_eig_B1 = -dt * nu_fourier * max_eig_Koverline + sqrt((dt * nu_fourier * max_eig_Koverline)^2 + 1);
        max_eig_B2 = -dt * nu_fourier * max_eig_Koverline - sqrt((dt * nu_fourier * max_eig_Koverline)^2 + 1);
        max_eig_B3 = 1;
        
        [max_eig_B, ~] = complex_max(max_eig_B1, max_eig_B2, max_eig_B3);
    
        stable = ((abs(max_eig_B) <= 1) || alpha_abs > 0) && nu_fourier < 1; % check that largest abs is less than 1
    end

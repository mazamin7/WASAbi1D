...
%   - p_next: the next pressure values (a column vector)
    ...
    elseif isDamped == true && exact_damped == false
        p_next_dct(n) = (2 - w2(n) * dt*dt)/(1 + alpha_abs*dt) ...
            .* p_curr_dct(n) - (1 - alpha_abs*dt) ...
            / (1 + alpha_abs*dt) * p_prev_dct(n) + dt*dt / (1 + alpha_abs*dt) * force_dct(n);

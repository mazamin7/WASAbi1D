...
t_axis_2 = linspace(0, len_t, 1e3);
x_axis_2 = linspace(0, len_x, 1e3);

% Evaluate the functions on a grid
[X, T] = meshgrid(x_axis_2, t_axis_2);
p_gt_2 = p_gt_fun(X, T);
v_gt_2 = v_gt_fun(X, T);

% Plot p_gt
figure(5);
surf(T, X, p_gt_2, 'EdgeColor', 'none', 'FaceColor', 'interp');
xlabel('Time [s]');
ylabel('Space [m]');
zlabel('Pressure');
...
% Plot v_gt
figure(6);
surf(T, X, v_gt_2, 'EdgeColor', 'none', 'FaceColor', 'interp');
xlabel('Time [s]');
ylabel('Space [m]');
zlabel('Velocity');

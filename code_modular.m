...
exact_damping = false; % Use exact damping (only works without interfaces)
dh = 1/2^3;
...
p_curr = zeros(N_x,1);
p_next = zeros(N_x,1);
q_next_dct = zeros(N_x,1); % for exact viscous damping
...
%     title('Gaussian Force Envelope')
    ...
    % Post-merge
    if choice == 2
        p_next = p_next + transmittivity^2 * (c0 * dt / dh)^2 * C * p_curr;
    end
